...
@section('PageContent')
    ...
    @if (auth()->user()->can('Branche_create-network'))
    <div style=" margin-bottom: 14px; position: relative; text-align: right; ">
        <a type="button" class="btn btn-primary" href="{{ route('networks.create') }}">@lang('Create new network provider')</a>
    </div>
    @endif
    @if ($lists->count() > 0)
                        ...
                        <table class="table project-list-table table-nowrap align-middle table-borderless">
                            ...
                            <tbody>
                                @foreach ($lists as $list)
                                    <tr>
                                        <td>
                                            <a _href="{{ route('networks.edit', $list->id) }}">
                                                {{ $list->name ?? '' }}
                                            </a>
                                        </td>
                                        <td>
                                        ...
                                        <td style="display: inline-flex;">
                                            @if (auth()->user()->can('Branche_update-network'))
                                            <a style="margin-right: 5px;" class="btn btn-outline-secondary btn-sm edit"
                                                href="{{ route('networks.edit', $list->id) }}">
                                                <i class="bx bx-pencil"></i>
                                            </a>
                                            @endif
                                            @if (auth()->user()->can('Branche_delete-network'))
                                            {!! action_table_delete(route('networks.destroy', $list->id), $list->id) !!}
                                            @endif
                                        </td>
                                    </tr>
                                @endforeach
